...
CREATE TABLE t_Type_Benchmark(
    ...
    lodTBN VARCHAR(255) NOT NULL PRIMARY KEY,
    TBN VARCHAR(255) NOT NULL,
    labelBenchmarkType VARCHAR(255) NOT NULL,
    descBenchmarkType TEXT NOT NULL,
...
M=n → 1 (‘Pass’)', 'pid_graph:3E109BBA',  NULL);
INSERT INTO t_Type_Benchmark ( lodTBN, TBN, labelBenchmarkType, descBenchmarkType, functionPattern, pattern, example, lodMTV, lodTBN_V) VALUES ('pid_graph:0A6A8E5A', 'S-S', 'String-String', 'Qualitative metric expressed as a string (derived from an opinion) is mapped to the closest semantically similar description of performance', 'TBC - Manual process at present', 'M(n) → P(n)', 'N/A', 'pid_graph:3E109BBA',  NULL);
INSERT INTO t_Type_Benchmark (lodTBN, TBN, labelBenchmarkType, descBenchmarkType, functionPattern, pattern, example, lodMTV, lodTBN_V) VALUES ( 'pid_graph:16108D9B', 'N-P', 'Normalised Value-Performance', 'The benchmark maps a value (which can be normalised) to a multi-faceted  assessment outcome (e.g. maturity level, performance level)', 'function applyBenchmark(M, testArray, outcomeArray) {
...
a<=M<b → 1 (‘Fair’)
b<=M<=1→ 2 (‘Good’)', 'pid_graph:3E109BBA',  NULL);
INSERT INTO t_Type_Benchmark (lodTBN, TBN, labelBenchmarkType, descBenchmarkType, functionPattern, pattern, example, lodMTV, lodTBN_V) VALUES ('pid_graph:6EBC59CF', 'N-B', 'Normalised Value-Binary', 'The benchmark maps a normalised integer or float value to binary assessment outcome (Pass/ Fail)', 'function applyBenchmark(value, benchmark) {
...
a<=M<=1 → 1 (‘Pass’)', 'pid_graph:3E109BBA', NULL);
INSERT INTO t_Type_Benchmark (lodTBN, TBN, labelBenchmarkType, descBenchmarkType, functionPattern, pattern, example, lodMTV, lodTBN_V) VALUES ( 'pid_graph:7085006F', 'B-B', 'Binary-Binary ', 'The benchmark maps a binary metric value to binary assessment outcome (Pass/ Fail). The inverse can also be done.', 'function applyBenchmark(value, invert) {
...
INSERT INTO t_Type_Benchmark (lodTBN,TBN, labelBenchmarkType, descBenchmarkType, functionPattern, pattern, example, lodMTV, lodTBN_V) VALUES ( 'pid_graph:AF8098B8', 'V-B-I', 'Value-Binary-Inverse', 'The benchmark maps a numeric value to a becnhmark - smaller or equal to the benhcmark is desired ', 'function applyBenchmark(value, benchmark) {
  ...
  // Return true if the value is equal to or larger than the benchmark, otherwise false
  return value <= benchmark;
}', NULL, NULL, 'pid_graph:3E109BBA',  NULL);

INSERT INTO t_Type_Benchmark (lodTBN, TBN, labelBenchmarkType, descBenchmarkType, functionPattern, pattern, example, lodMTV, lodTBN_V) VALUES ( 'pid_graph:BBB9ABD5', 'ASO', 'Assessment Outcome', 'A complex benchmark is applied, with an array of values evaluated against benchmark values, and compliance is either mandatory or optional.', 'function evaluateMetrics(metrics, benchmarks, imperatives) {
  // Validate input parameters
...
INSERT INTO t_Type_Benchmark( lodTBN, TBN, labelBenchmarkType, descBenchmarkType, functionPattern, pattern, example, lodMTV, lodTBN_V) VALUES ( 'pid_graph:C4D0F2B1', 'V-B', 'Value-Binary', 'The benchmark maps a numeric value to a becnhmark - greater or equal to the benhcmark is desired ', 'function applyBenchmark(value, benchmark) {
  // Check if the input parameters are integers
  if (typeof value !== ''number'' || typeof benchmark !== ''number'') {
    throw new Error(''Both parameters must be numbers.'');
  }
