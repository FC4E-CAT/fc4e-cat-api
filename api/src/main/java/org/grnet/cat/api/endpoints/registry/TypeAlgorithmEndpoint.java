...
public class TypeAlgorithmEndpoint {
    ...
    public Response listTypeAlgorithm(
        ...
        var typeAlgorithm = typeAlgorithmService.getTypeAlgorithmAll(page - 1, size, enabled, uriInfo);
    ...
    @Tag(name = "Metrics")
    @Operation(
            summary = "Update TypeAlgorithm",
            description = "Updates an existing TypeAlgorithm item."
    )
    @APIResponse(
            responseCode = "200",
            description = "TypeAlgorithm was updated successfully.",
            content = @Content(schema = @Schema(
                    type = SchemaType.OBJECT,
                    implementation = TypeAlgorithmResponseDto.class)))
    @APIResponse(
    ...
    public Response updateTypeAlgorithm(
            @Parameter(
                    description = "The ID of the TypeAlgorithm to update.",
                    required = true,
                    example = "pid_graph:9F1A6267",
                    schema = @Schema(type = SchemaType.STRING))
            @PathParam("id")
            @Valid @NotFoundEntity(repository = TypeAlgorithmRepository.class, message = "There is no Test Algorithm with the following id:") String id,
            @Valid TypeAlgorithmUpdateDto request) {

        var updatedDto = typeAlgorithmService.updateTypeAlgorithm(id, utility.getUserUniqueIdentifier(), request);
