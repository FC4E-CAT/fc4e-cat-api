...
public class AssessmentsV2Endpoint {
    ...
    public Response createZenodoDeposit(@Parameter(
        ...
        ZenodoAssessmentInfoResponse zenodoAssessmentInfoResponse = null;
        try {
            zenodoAssessmentInfoResponse = zenodoService.publishAssessment(id, pdfFile);
        } catch (IOException | InterruptedException e) {
            throw new RuntimeException(e);
        }
        return Response.ok().entity(zenodoAssessmentInfoResponse).build();
    }

    @Tag(name = "Assessment")
    @Operation(
            summary = "Zenodo publish a deposit.",
            description = "Allows a user to publish an deposit to zenodo.")
    @APIResponse(
            responseCode = "200",
            description = "Deposit published successfully.",
            content = @Content(schema = @Schema(
                    type = SchemaType.OBJECT,
                    implementation = InformativeResponse.class)))
    @APIResponse(
    ...
    @SecurityRequirement(name = "Authentication")
    @POST
    @Path("/zenodo/publish/deposit/{id}")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_OCTET_STREAM) // This consumes binary data
    @Registration
    public Response publishZenodoDeposit(@Parameter(
        ...
        var message = zenodoService.publishDepositToZenodo(id);
        var response=new InformativeResponse();
        response.code=200;
        response.message=message;
        return Response.ok().entity(response).build();
