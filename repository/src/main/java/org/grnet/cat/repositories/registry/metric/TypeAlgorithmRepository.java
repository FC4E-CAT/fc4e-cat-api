...
public class TypeAlgorithmRepository implements Repository<TypeAlgorithm, String> {
     ...
     */
    public PageQuery<TypeAlgorithm> fetchTypeAlgorithmByPage(int page, int size, Boolean enable) {

        var joiner = new StringJoiner(" ");
        joiner.add("from TypeAlgorithm ta");
        joiner.add("where 1=1");

        var map = new HashMap<String, Object>();

        if (enable != null) {
            joiner.add("and ta.enabled = :enabled");
            map.put("enabled", enable);
        }

        joiner.add("order by ta.lastTouch DESC, ta.id ASC");
