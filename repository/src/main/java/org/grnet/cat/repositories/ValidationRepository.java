...
public class ValidationRepository implements Repository<Validation, Long> {
    ...
    @Inject
    RegistryActorRepository actorRepository;
    ...
    public PageQuery<Validation> fetchValidationsByPage(String search, String sort, String order, String type, String status, int page, int size) {
        ...
        if (!Objects.isNull(type)) {
            ...
            joiner.add("and validation.registryActor.id = :id");
    ...
    public Long countApprovedValidationsByUserId(String userId) {

        return count("from Validation v where v.user.id = ?1 and v.status = ?2", userId,ValidationStatus.APPROVED);
    }
